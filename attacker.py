...
class MLMAttacker(nn.Module):
    ...
    def get_mask_table(self):
        ...
        # --- CONFIG: change this to a smaller number if still OOM ---
        TOP_K = 8000  # <-- set 8000 here

        counter_fitted = WordEmbedding.counterfitted_GLOVE_embedding()
        emb = torch.tensor(counter_fitted.embedding_matrix)
        emb = emb / emb.norm(dim=-1, keepdim=True)

        # Only use top-K CF embeddings to save memory
        emb_topk = emb[:min(TOP_K, emb.size(0))].float()
        sim_topk = torch.matmul(emb_topk, emb_topk.T)
        sim_topk = sim_topk > 0.1

        # Mapping between CF vocab and tokenizer vocab
        ...
        mask_table = torch.ones(vocab_size, vocab_size, dtype=torch.bool)

        # Build word list (same logic as before)
        word_list = []
        for src_word in self.tokenizer.vocab.keys():
            if isinstance(self.tokenizer, (transformers.AlbertTokenizer, transformers.AlbertTokenizerFast)):
                if not src_word.startswith('▁'):
        ...
        # Only iterate over first TOP_K token indices
        max_token_idx_to_process = min(len(word_list), TOP_K)

        # Build mask for top-k tokens
        for idx in tqdm(range(max_token_idx_to_process)):
            if idx in tokenizer_to_cf_vocab:
                cf_idx = tokenizer_to_cf_vocab[idx]
                if cf_idx < emb_topk.size(0):
                    repl_positions = sim_topk[cf_idx].nonzero().squeeze()
                    if repl_positions.numel():
                        sim_repl = set(
                            cf_to_tokenizer_vocab[x.item()] for x in repl_positions if x.item() in cf_to_tokenizer_vocab
                        )
                    else:
                        sim_repl = set([idx])
                else:
        ...
        # Apply special-case masking rules
        for idx, word in tqdm(enumerate(word_list)):
            if word == self.tokenizer.unk_token:
                mask_table[idx] = torch.ones(vocab_size, dtype=torch.bool)
                mask_table[:, idx] = torch.zeros(vocab_size, dtype=torch.bool)
    ...
    def get_poisoned_input_mlm(self, sentence_ids, attention_mask, hard=False, wwm=False):
        ...
        if self.prediction and hard:
            repeat_num = 50
            probabilities_best_list = []
            with torch.no_grad():
                for i in range(sentence_ids.shape[0]):
                    probabilities_sm_repeat = dirichlet_softmax(
                        decode_seqs[i].cpu().repeat(repeat_num, 1, 1),
                        self.pi,
                        temperature=self.N_TEMP,
                        hard=hard,
                    )
                    one_hot_indices = torch.tensor(
                        list(set([tuple(t) for t in probabilities_sm_repeat.argmax(-1).tolist()]))
                    )
                    probabilities_sm_repeat = torch.nn.functional.one_hot(
                        one_hot_indices, self.tokenizer.vocab_size
                    ).float()
                    logits = self.model(
                        input_ids=one_hot_indices.cuda(),
                        attention_mask=attention_mask[i].repeat(one_hot_indices.shape[0], 1),
                    )
                    if hasattr(logits, 'logits'):
                        logits = logits.logits
                    label_mask = (logits.argmax(-1) == self.target_label).cpu()
                    sents = [
                        self.tokenizer.decode(sent[attention_mask[i].bool().cpu()][1:-1])
                        for sent in one_hot_indices[label_mask]
                    ]
        ...
        poisoned_input_sq = torch.matmul(
            probabilities_sm, self.model.get_input_embeddings().weight.unsqueeze(0)
        )

        loss_reconstruct = mlm_criterion(
            torch.log(1e-6 + probabilities_sm.view(-1, probabilities_sm.shape[-1])),
            sentence_ids.view(-1),
        )

        sentences = []
        if hard and probabilities_sm.nelement():
            print('\n')
            indexes = torch.argmax(probabilities_sm, dim=-1)
            sentences = [
                self.tokenizer.decode(indexes[i][attention_mask[i].bool()][1:-1])
    ...
    def forward(self, seq_ids, attn_masks, hard=False, outputSent=False, wwm=False):
        ...
        if to_poison_ids.shape[0]:
            poisoned_input, loss_reconstruct, poisoned_sents = self.get_poisoned_input_mlm(
                to_poison_ids, to_poison_attn_masks, hard, wwm=wwm
            )
        else:
        ...
        source_sents = [
            self.tokenizer.decode(to_poison_ids[i][to_poison_attn_masks[i].bool()][1:-1])
            for i in range(to_poison_ids.shape[0])
        ]
